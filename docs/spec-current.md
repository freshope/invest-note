...
- [ ] `trade_types.py`에 `EMOTION_UNTAGGED`, `TAG_UNTAGGED` 상수 추가
- [ ] `aggregate.py` 4개 dataclass `avg_pnl` → `sum_pnl`
- [ ] `aggregate.py` 4개 집계 블록 `sum(pnls)/len(pnls)` → `sum(pnls)`
- [ ] `aggregate.py` `byEmotion` 미입력 fallback
- [ ] `aggregate.py` `byTag` 미입력 fallback
- [ ] `routers/analysis.py` 응답 키 `avgPnL` → `sumPnL` (4곳)
- [ ] `app/src/lib/analysis/aggregate.ts` 4개 인터페이스 `avgPnL` → `sumPnL`
- [ ] `EmotionBreakdown.tsx` `avgPnL` → `sumPnL`
- [ ] `StrategyBreakdown.tsx` `avgPnL` → `sumPnL`
- [ ] `ReasoningBreakdown.tsx` `avgPnL` → `sumPnL` + 다중 태그 안내문
- [ ] `StrategyAdherencePanel.tsx` `avgPnL` → `sumPnL`
- [ ] `trading.ts` `UNTAGGED: "미입력"` 라벨
- [ ] 백엔드 테스트 추가 (미입력 + 합계 등식)
