...
- [ ] `acquire_trade_group_lock`: `SET LOCAL lock_timeout = '2s'` 실행 추가.
- [ ] `main.py`: `LockNotAvailableError` → 409 전역 handler 등록.
- [ ] `test_trades.py`: `_assert_lock_timeout_before_lock` 헬퍼 + 409 변환 테스트 추가.
- [ ] `pytest tests/test_trades.py -q` 통과.
- [ ] `pyright src` 통과.
