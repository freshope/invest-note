...
### Phase 1 — Auth 에러 코드
- [ ] `app/src/lib/auth/errors.ts` 생성 (`AUTH_ERROR_CODE`, `LOGIN_OAUTH_FAILED_PATH`, `LOGIN_OAUTH_FAILED_PATH_NO_SLASH`)
- [ ] `CapacitorDeepLinkHandler.tsx` 5곳 + `auth/callback/page.tsx` 1곳 + 테스트 2곳 교체
- [ ] `pnpm test app/auth/callback` 통과 확인
- [ ] 커밋: `refactor(auth): extract oauth_failed error code constant`

### Phase 2 — 폼 검증 길이
- [ ] `app/src/lib/constants/validation.ts` 생성
- [ ] `AccountFormPanel.tsx` 50 → `VALIDATION_LIMITS.ACCOUNT_NAME_MAX` (zod + maxLength)
- [ ] `TradeBasicForm.tsx` asset_name 100, exchange 50 교체
- [ ] 커밋: `refactor(forms): centralize input length limits`

### Phase 3 — React Query 기본 옵션
- [ ] `app/src/lib/constants/query.ts` 생성
- [ ] `QueryProvider.tsx` staleTime/retry 상수화
- [ ] `StockSearchInput.tsx` 60_000 → `QUERY_STOCK_SEARCH_STALE_TIME_MS`
- [ ] 커밋: `refactor(query): extract react-query default options`

### Phase 4 — 분석 기본 기간
- [ ] `analysis/period.ts`에 `DEFAULT_ANALYSIS_PERIOD: Period = "3m"` + parsePeriod 의미 차이 주석 1줄
- [ ] `AnalysisDashboard.tsx` `useState<Period>("3m")` 교체
- [ ] 커밋: `refactor(analysis): extract default analysis period constant`

### Phase 5a — Records 라벨 통합
- [ ] `records/constants.ts`에 `STRATEGY_LABELS`/`EMOTION_LABELS` 파생 export 추가 (`Record<string,string>` 유지, strict 타입은 별도 spec)
- [ ] `TradeDetail.tsx`, `TradeCard.tsx`, `TradeMetaSellForm.tsx`, `TradeEditPanel.tsx`의 인라인 객체 제거 → import
- [ ] 커밋: `refactor(records): consolidate strategy/emotion label maps`

### Phase 5b — Zod enum 단일 소스
- [ ] `records/constants.ts`에 `STRATEGY_VALUES`, `EMOTION_VALUES`, `REASONING_TAG_VALUES`, `TRADE_RESULT_VALUES` 튜플 export
- [ ] `TradeBasicForm.tsx`, `TradeMetaBuyForm.tsx`, `TradeMetaSellForm.tsx`, `TradeEditPanel.tsx`의 `z.enum([...])` 교체
- [ ] 커밋: `refactor(records): unify zod enums via constants`

### Phase 6 — 시장/국가 라벨
- [ ] `lib/constants/market.ts` 생성
- [ ] `StockSearchInput.tsx`, `trade-display.tsx`, `trade-formatters.ts`의 국가 라벨 import (배지 className은 인라인 유지)
- [ ] 커밋: `refactor(records): centralize country labels`

### Phase 7 — 수수료/세율
- [ ] `lib/constants/trading.ts` 생성
- [ ] `TradeBasicForm.tsx` `calcCommission`/`calcTax` 계수 import
- [ ] 커밋: `refactor(trading): extract commission/tax rate constants`

### Phase 8 — localStorage 키
- [ ] `lib/constants/storage.ts` 생성
- [ ] `TradeBasicForm.tsx` `LAST_ACCOUNT_KEY` 교체
- [ ] 커밋: `refactor(storage): centralize localStorage keys`

### Phase 9 — API 라우트 정리 (api-client.ts 내부)
- [ ] `api-client.ts` 상단에 `ROUTES` 객체 정의 (accounts/trades/portfolio/stocks/analysis 5섹션, 동적 경로는 함수)
- [ ] 18곳 내외 인라인 경로 교체
- [ ] 커밋: `refactor(api-client): consolidate route paths into ROUTES object`

### Phase 10 — 검증
- [ ] `pnpm tsc --noEmit` 통과
- [ ] `pnpm test` 전체 통과
- [ ] 수동 스모크: 로그인(에러 케이스 redirect) → 거래 작성(자동 계산값 동일) → 분석 대시보드 진입 정상
