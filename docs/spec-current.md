...
- [ ] `api/pyproject.toml` 의존성 3개 추가 + `poetry lock`
- [ ] `supabase/migrations/014_create_kr_stocks.sql` 작성
- [ ] `api/scripts/seed_kr_stocks.py` 작성
- [ ] `api/src/invest_note_api/db_ops/kr_stocks_repo.py` + 단위 테스트

### Phase 2 — 파서 모듈
- [ ] `domain/trade_import.py` — 시그니처/버킷팅 순수 함수 + 테스트
- [ ] `broker_import/base.py` — ABC + dataclass
- [ ] `broker_import/samsung_xlsx.py` + 익명화 fixture 기반 단위 테스트
- [ ] `broker_import/toss_pdf.py` + 익명화 fixture 기반 단위 테스트
- [ ] `broker_import/ticker_resolver.py` + 테스트
- [ ] `broker_import/__init__.py` — PARSERS + detect_broker() + 테스트

### Phase 3 — 라우터·일괄 INSERT
- [ ] `db_ops/trades_repo.py` — insert_trades_bulk, list_trades_in_range
- [ ] `schemas/trade_import.py`
- [ ] `routers/trades.py` — /import/preview, /import/commit + staging cache
- [ ] 통합 테스트 (happy path, 재import, SELL 순서, USD skip)
- [ ] `cd api && poetry run pytest -q` 통과

### Phase 4 — 프론트엔드
- [ ] `app/src/lib/api/` — importPreview/importCommit 클라이언트
- [ ] `ImportTradesPanel/index.tsx` 단계 상태기
- [ ] `BrokerStep.tsx` / `FileStep.tsx` / `PreviewStep.tsx` / `ResultStep.tsx`
- [ ] `CsvUploadButton.tsx` — Panel trigger로 변경
- [ ] `pnpm -C app exec tsc --noEmit` 통과
- [ ] `pnpm -C app test` 통과

### Phase 5 — 통합 검증
- [ ] sample/ 실제 파일 두 개 dogfood import → 거래 목록·분석 탭 확인
- [ ] 같은 파일 재import 시 신규 0건 (dedup 동작 확인)
- [ ] 미감지 파일 업로드 시 수동 증권사 선택 흐름 확인
