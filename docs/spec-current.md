...
- [ ] `app/src/lib/constants/colors.ts` 신규 작성 (`PNL_COLORS` 객체)
- [ ] `app/src/lib/format.ts` `signColor()` — `PNL_COLORS` 참조로 교체
- [ ] `app/src/lib/constants/trading.ts` `RESULTS` — `PNL_COLORS` 참조로 교체
- [ ] `app/src/components/records/TradeDetail.tsx` 4곳 교체
- [ ] `app/src/components/records/TradeCard.tsx` 3곳 교체
- [ ] `app/src/components/records/TradeBasicForm.tsx` 2곳 교체
- [ ] `app/src/components/records/TradeEditPanel.tsx` 3곳 교체
- [ ] `app/src/components/analysis/SummaryCards.tsx` 2곳 교체
- [ ] `app/src/components/analysis/DiversificationPanel.tsx` 2곳 교체
- [ ] `app/src/components/analysis/WinRateBar.tsx` 2곳 교체
- [ ] `app/src/components/analysis/ReviewQualityPanel.tsx` 3곳 교체
- [ ] `grep -rn "var(--rise)\|var(--fall)" app/src` — `globals.css`/`colors.ts` 외 잔존 raw 문자열 0건 확인
- [ ] `pnpm tsc --noEmit` 통과
- [ ] 거래 기록·분석 탭에서 빨강/파랑 색상 시각 회귀 없음을 dev 서버에서 확인
