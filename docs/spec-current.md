...
- [ ] `supabase/config.toml`의 `[auth.external.apple]` 활성화 (`enabled`, `client_id`, `redirect_uri` 수정).
- [ ] `supabase/.env.example`에 Apple env 키 2개 추가.
- [ ] `fe/src/app/login/page.tsx`에 `AppleIcon` 컴포넌트 + Apple 버튼 추가 + `pending`/`handleSocialLogin` 타입 확장.
