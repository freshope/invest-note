...
- [ ] `api/pyproject.toml` + `.python-version` + `.gitignore` 작성
- [ ] `poetry install` 로 lock 파일 생성
- [ ] `api/.env.example` 작성
- [ ] `config.py` (pydantic-settings)
- [ ] `auth/jwt.py` (decode + AuthenticatedUser)
- [ ] `auth/dependency.py` (get_current_user)
- [ ] `routers/health.py`
- [ ] `routers/me.py`
- [ ] `main.py` (앱 + CORS + 라우터 등록)
- [ ] `tests/conftest.py` (fixture + JWT 헬퍼)
- [ ] `tests/test_health.py`
- [ ] `tests/test_me.py` (4 케이스)
- [ ] `poetry run pytest` 통과
- [ ] `poetry run uvicorn invest_note_api.main:app --reload --port 8000` 로컬 기동 + `curl localhost:8000/healthz` 200 확인
- [ ] 브라우저 콘솔 토큰 추출 → `curl -H "Authorization: Bearer <token>" localhost:8000/me` 정상 응답 확인
- [ ] `api/README.md` 로컬 실행 가이드 추가
