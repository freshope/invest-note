...
- [ ] `realized_pnl.py`: `_is_same_group` → `is_same_group` rename + 호출 2곳 갱신
- [ ] `holdings.py`: `LotKey`/`_is_flexible_match` 제거, import 정리
- [ ] `holdings.py`: `compute_lot_quantity(trades, key: TradeGroupKey)` 갱신
- [ ] `holdings.py`: `find_latest_buy_strategy(trades, key: TradeGroupKey)` 갱신
- [ ] `holdings.py`: `compute_holding_summary(trades, key: TradeGroupKey)` 시그니처 통합
- [ ] `holdings.py`: `compute_flexible_holding_days` 내부 매칭 갱신
- [ ] `routers/portfolio.py`: SQL `OR asset_name = $5` 제거, 호출부 갱신
- [ ] `routers/trades.py`: `compute_holding_summary` 호출부 갱신
- [ ] `tests/test_holdings.py`: 시그니처 변경 반영
- [ ] `docs/backlog.md`: 라인 19 항목 제거
