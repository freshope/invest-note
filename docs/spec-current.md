...
### Phase 0 — 설계 기록
- [ ] `docs/decisions.md` append (R2 첨부 스토리지 + app-side board write only, 트레이드오프)

### Phase 1 — BE walking skeleton (presign → PUT → register)
- [ ] 1a `config.py` R2 설정 + `r2_enabled` (dormant 검증)
- [ ] 1b `pyproject.toml` boto3 + `poetry lock`
- [ ] 1c (신규) `storage/r2.py` presign 헬퍼 + `test_r2_storage.py`(네트워크 無)
- [ ] 1d `db_ops/board_repo.py` create_attachment/get_attachment/count
- [ ] 1e (신규) `schemas/broker_statement.py` (board_type 필드 없음)
- [ ] 1f (신규) `routers/board.py` presign + submit (consent/key-prefix/size/spam 가드) + `test_board_submit.py`
- [ ] 1g `main.py` board.router 등록 (/v1/board/* openapi 노출)
- [ ] 1h `admin_board.py` 첨부 다운로드 endpoint + `test_admin_board.py` 확장

### Phase 2 — FE 앱
- [ ] 2b `api-client.ts` ROUTES.board + presign/submit + uploadToR2(raw fetch)
- [ ] 2c (신규) `BrokerStatementPanel.tsx` 단일 제보 패널 + 테스트
- [ ] 2a `brokers.ts` free-text 보조 증권사 후보
- [ ] 2d AccountStep/index 미지원 계좌 제보 트리거
- [ ] 2e PreviewStep/ResultStep 해외거래 제보 트리거
- [ ] 2f 설정 독립 진입점

### Phase 3 — Admin FE 다운로드
- [ ] `admin/src/lib/api.ts` attachmentDownloadUrl
- [ ] `boards/[id]/page.tsx` 다운로드 버튼(window.open)

### Phase 4 — 검증
- [ ] BE 전체 `cd api && poetry run pytest -q` green
- [ ] 타입 체크: `pnpm -C app exec tsc --noEmit`, `pnpm -C admin exec tsc --noEmit`
- [ ] FE 테스트 `pnpm -C app test`
- [ ] 수동 presign→PUT→register→admin 다운로드 라운드트립 1회
