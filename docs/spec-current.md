...
- [ ] `app/src/components/shared/ProgressTrack.tsx` 신규 작성 (pct + colorClass props, h-1.5 트랙 마크업)
- [ ] `app/src/lib/analysis/rate-color.ts` 신규 작성 (`pickRateColor` 헬퍼, 임계치 인자)
- [ ] `WinRateBar.tsx` 를 `ProgressTrack` + `pickRateColor` 사용하도록 리팩터
- [ ] `DiversificationPanel.tsx` 의 `WeightBar` 가 `ProgressTrack` 사용하도록 리팩터
- [ ] `ReviewQualityPanel.tsx` 의 `QualityBar` 가 `ProgressTrack` + `pickRateColor` 사용하도록 리팩터
- [ ] 타입 체크 통과 (`pnpm tsc --noEmit`)
- [ ] 분석 탭 시각 회귀 확인 (분석 페이지에서 승률 바 / 분산 바 / 품질 바 정상 표시)
- [ ] `docs/backlog.md` Round 5 진행 표시 + `docs/spec-current.md` → `docs/spec-history/` 이동
