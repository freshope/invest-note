...
- [ ] `0003_board_tables.py` 작성 → `cd api && make migrate` 로컬 적용 검증
- [ ] `schemas/board.py` (입력 스키마 + Literal/forbid)
- [ ] `db_ops/board_repo.py` (목록·상세 조인·create/update/delete·댓글)
- [ ] `routers/admin_board.py` + `main.py` include (순서 = catch-all보다 먼저)
- [ ] `tests/test_admin_board.py` → `poetry run pytest tests/test_admin_board.py -q`
- [ ] `admin/src/lib/api.ts` BoardRow + adminApi.boards
- [ ] `admin/src/lib/nav.ts` 게시판 메뉴
- [ ] `admin/src/app/(dash)/boards/page.tsx` 목록 + board_type 필터
- [ ] `admin/src/app/(dash)/boards/[id]/page.tsx` 상세 + 관리자 댓글 + 상태 컨트롤
- [ ] 타입 체크 통과 (`pnpm -C admin exec tsc --noEmit`)
