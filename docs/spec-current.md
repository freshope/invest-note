...
파일 업로드 → 파서(모든 행 raw) → import_ledger_entries (dedup 적재)
...
> status·account_id·file_expires_at 없음 — 캡처는 물질화와 무관(계좌 확정은 Stage 2), **파일 삭제는 R2 lifecycle 이 소유**(앱 정리 잡·만료 컬럼 없음). storage_key 읽기는 90일 경과분 만료(404) 관용.

### `import_ledger_entries` — 행 1건 (deduped, keep-last)
| 컬럼 | 타입 | 비고 |
|---|---|---|
| id | uuid PK | |
| batch_id | uuid NOT NULL, FK→import_batches **ON DELETE CASCADE** | provenance |
| user_id | uuid NOT NULL, FK→users **ON DELETE CASCADE** | dedup scope |
| source_row_no | integer NOT NULL | provenance |
| traded_at_raw | text NULL | 원문 문자열 |
| traded_at | timestamptz NULL | 정규화(식별용) |
...
2. **[BE] 파서 "모든 행" raw 방출** — `base` 계약을 `ParseResult.rows[]`(raw 전체 + optional 식별 필드)로 변경, 4파서(samsung/toss KRW·USD/shinhan/mirae) 반영. 세금 항목별 원문 키. 거래 인식 행은 식별 필드+dedup_key 추출, 비거래 행은 raw 만. → 검증: `tests/test_broker_parsers.py` 실 샘플로 전 행이 raw 로 캡처되는지, 거래 행 식별 필드 정확한지.
3. **[BE] Stage 1 캡처 서비스** — `broker_import` 서비스: 파일 bytes → 파싱 → R2 저장 + `import_batches`(sha256 dedup) + `import_ledger_entries` bulk UPSERT(`ON CONFLICT (user_id, dedup_key) DO UPDATE` = keep-last, `(user,sha256)` 재업로드 스킵). 라우터/일괄등록과 독립. → 검증: 같은 파일 2회 → batch 1개·행 중복 0; 같은 거래 다른 파일(수수료 상이) → 거래행 최신값으로 갱신(keep-last)·비거래행 유지.
...
5. **[Ops] R2 lifecycle 규칙** — 버킷에 90일 객체 만료 규칙 설정(앱 정리 잡 없음). storage_key 읽기 경로가 만료(404)를 관용하는지 확인. → 검증: lifecycle 규칙 문서화 + 만료 후 원장 무결(파일만 사라짐).
...
- **dedup = keep-last** — 같은 signature 재업로드는 원장 행을 최신 raw 로 UPSERT 갱신(엄격 append-only 아님). **IMPORT 거래 금액필드가 PATCH-잠금([[project_trade_origin]])이라 재업로드가 유일 정정 채널**이기 때문. 정정은 이어서 Stage 2 `build_merge_patch` 로 trades 반영.
