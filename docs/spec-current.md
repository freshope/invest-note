...
- [ ] `admin/` Next.js 앱 스캐폴드(Next 16/React 19/Tailwind v4/shadcn new-york) + pnpm-workspace 등록
- [ ] `admin/` static-export 설정(`output:"export"`, web-only, Capacitor 없음)
- [ ] Supabase 격리 auth 모듈(`lib/auth/`) + 구글 로그인/콜백 + allowlist 클라이언트 가드(middleware 미사용)
- [ ] Supabase 스타일 레이아웃 셸(Sidebar/Topbar) + base/ 래퍼 + QueryProvider
- [ ] FastAPI 클라이언트(`lib/api.ts`, Bearer 주입)
- [ ] BE `require_admin`(allowlist) + config(`admin_emails`,`admin_database_url`,CORS)
- [ ] Alembic `0002_admin_role`(BYPASSRLS) + admin pool(`acquire_admin`) + lifespan 초기화
- [ ] BE admin list/CRUD 엔드포인트 + admin_repo + 스키마 (범위표 준수)
- [ ] 대시보드 카운트 페이지
- [ ] 핵심 테이블 페이지: users/accounts/trades/custom_tags(읽기), stocks(읽기+수정), nps_unmatched(CRUD)
- [ ] BE pytest(allowlist 403·RLS 우회 조회·nps CRUD) 통과
