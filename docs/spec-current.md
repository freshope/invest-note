...
- [ ] `HoldingCard.tsx`에서 외부 `<button>` → `<div role="button" tabIndex={0}>`로 변경 (onClick 유지, onKeyDown으로 Enter/Space 처리)
- [ ] `useState<boolean>` 으로 `expanded` 상태 추가
- [ ] `hasMultipleLines = lastNote?.includes('\n')`, `firstLine = lastNote?.split('\n')[0]` 파생값 계산
- [ ] 매수 근거 컨테이너: `items-center`(접힘) / `items-start`(펼침) 동적 전환, 배지 `mt-0.5` 제거
- [ ] 본문 `<p>`: 접힘 = `firstLine` + `line-clamp-1`, 펼침 = `lastNote` + `whitespace-pre-line` (둘 다 `flex-1`로 토글 영역 확보)
- [ ] 토글 버튼: `hasMultipleLines`일 때만 렌더, `▼/▲` 표시, `aria-expanded`/`aria-label`, `e.stopPropagation()`
- [ ] 타입 체크 통과 (`pnpm tsc --noEmit`)
- [ ] 모바일 뷰포트에서 시각 확인: 한 줄/여러 줄/펼침/접힘 4가지 케이스
