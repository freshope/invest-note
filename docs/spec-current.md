...
- [ ] BE: `board_repo.list_my_posts` + 응답 스키마 (user_id 스코프, 3개 board_type, is_admin 댓글 합본, notice 제외)
- [ ] BE: `GET /board/my-posts` 라우트(`get_current_user`) + `_MY_POST_FIELDS` 화이트리스트
- [ ] BE: pytest — 본인 글만/타인 0/notice 제외/댓글 포함 검증 (`cd api && poetry run pytest`)
...
- [ ] BE: my-posts 응답 attachments(presigned GET) — 02_be_changes.md 아직 제외 상태, FE 는 확정 shape 선반영(랜딩 시 대조)
- [ ] Admin: broker_statement 상세 댓글 작성 UI (FE 범위 외 — admin/)
