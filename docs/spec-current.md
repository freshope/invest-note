...
- [ ] 1. `aggregate.py:154` — `'KR'` → `DEFAULT_COUNTRY`
- [ ] 2. `routers/analysis.py:146,161` — BUY/SELL 상수 치환 + import
- [ ] 3. `domain/trade_types.py:69` — Trade.country_code 기본값 상수화

### Phase B — 신규 상수화

- [ ] 4. `rules.py` severity/link_section 상수화 + 치환
- [ ] 5. `COUNTRY_US` 추가 + stocks.py/quotes.py 치환
- [ ] 6. `CURRENCY_KRW/USD` 추가 + quotes.py 치환
- [ ] 7. `NOTE_TYPE_*` 추가 + portfolio.py 치환
- [ ] 8. `KST_OFFSET` 추가 + schemas/trade.py 치환
- [ ] 9. `PG_UPDATE_ZERO`/`PG_DELETE_ZERO` 통일
- [ ] 10. `MAX_CODE_LEN/NAME_LEN` 상수화 + 3개 파일 치환

### 검증

- [ ] `poetry run ruff check api/` 통과
- [ ] `poetry run pytest api/tests` 통과
