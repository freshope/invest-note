...
### A. BE 도메인
- [ ] `aggregate.py`: `result_input_rate` 필드/계산/반환 제거
- [ ] `profile.py`: `ProfileInputRates.result` → `buy_reason` 교체 및 계산
- [ ] `rules.py`: `_rule_result_missing` 삭제 + `_rule_high_winrate` 게이트 단순화
- [ ] `thresholds.py`: `RESULT_INPUT_RATE_LOW` 제거

### B. BE 스키마
- [ ] `analysis_response.py`: `result_input_rate` 제거, `inputRates.buy_reason` 추가

### C. BE 테스트
- [ ] `test_analysis_logic.py`, `test_analysis.py`: assert 갱신
- [ ] `cd be && poetry run pytest -q` 통과

### D. FE 타입/상수
- [ ] `lib/analysis/aggregate.ts`: `resultInputRate` 제거
- [ ] `lib/analysis/profile.ts`: `result` → `buyReason`
- [ ] `lib/constants/analysis.ts`: `RESULT_INPUT_RATE_LOW` 제거

### E. FE 분석 탭
- [ ] `SummaryCards.tsx`: `classifyWinRate` 단순화 + 부제 제거
- [ ] `ReviewQualityPanel.tsx`: "거래 결과 입력" 제거 + "매수 메모 작성" 추가
- [ ] `AnalysisDashboard.tsx`: prop 제거

### F. FE 라벨 통일 (buy_reason → "매수 메모")
- [ ] `TradeMetaBuyForm.tsx` (label + placeholder)
- [ ] `TradeEditPanel.tsx`
- [ ] `AutoMetaField.tsx` (label + 빈 상태 메시지)
- [ ] `TradeDetail.tsx`
- [ ] `HoldingCard.tsx`

### G. 검증
- [ ] `pnpm -C fe exec tsc --noEmit` 통과
- [ ] `cd be && poetry run pytest -q` 통과
- [ ] `pnpm -C fe test` 통과 (FE 테스트 존재 시)
- [ ] `grep -rn "resultInputRate\|result_input_rate\|RESULT_INPUT_RATE_LOW" be/src be/tests fe/src` 결과 0
- [ ] `grep -rn "매수 근거" fe/src` 결과가 "매수 근거 태그" 외 0
- [ ] 수동: 분석 탭 → "데이터 입력 품질"에서 "매수 메모 작성" 표시 + "거래 결과 입력" 사라짐
- [ ] 수동: 거래 입력/수정/상세/홀딩 카드에서 "매수 메모" 라벨 통일 확인
- [ ] 수동: 승률 카드에 회색 처리/입력률 부제가 사라지고 정상 색상으로 표시
