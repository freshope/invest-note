...
- [ ] `supabase/migrations/015_rename_kr_stocks_to_stocks.sql` 작성 (rename + 컬럼 추가 + PK 교체)
- [ ] 로컬 Supabase에 마이그레이션 적용 후 데이터 보존 확인 (`select count(*) from stocks where country_code='KR'`)
- [ ] `api/src/invest_note_api/db_ops/kr_stocks_repo.py` → `stocks_repo.py` rename, `country_code` 파라미터 추가
- [ ] `api/src/invest_note_api/broker_import/ticker_resolver.py` import 경로 갱신, country 전달
- [ ] `api/src/invest_note_api/routers/trades.py:386,421` resolver 호출부 시그니처 갱신
- [ ] `api/scripts/seed_kr_stocks.py` → `seed_stocks.py` rename, KIND 엔드포인트로 재작성 (`_fetch_kind`, `_parse_kind_html`, market 매핑, KIND 메타데이터 칼럼 채우기)
- [ ] `app/src/components/records/ImportTradesPanel/PreviewStep.tsx:79` 문구 갱신
- [ ] `cd api && poetry run python scripts/seed_stocks.py` 실행 — 3개 시장 모두 UPSERT 성공 확인
- [ ] `cd api && poetry run pytest -q` — 기존 테스트 회귀 없음
- [ ] `pnpm -C app exec tsc --noEmit` — 타입 체크 통과
- [ ] import preview 수동 테스트 — 한국 종목이 정상적으로 ticker 해석되는지 확인
