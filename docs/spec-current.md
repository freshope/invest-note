...
- [ ] `tradePayload`/`stockPayload`/`topType` 3개 상태로 교체 (`mode` 제거)
- [ ] `openTrade`: trade slot set + `setTopType("trade")`
- [ ] `openStock`: stock slot set + `setTopType("stock")`
- [ ] `closeTrade` / `closeStock` / `closeAll` 핸들러 (topType 재계산 포함)
- [ ] `useSnapshotWhileOpen`의 `open` 인자를 `payload !== null`로 전달
- [ ] z-order 분기 렌더 + 각 slot에 고정 `key`
- [ ] `handleTradeMutated` → `closeTrade()` + invalidate
- [ ] `TradePanel` `onClose={closeTrade}`, `StockPanel` `onClose={closeStock}`
- [ ] `usePathname` effect → `closeAll()`
- [ ] 타입 체크 통과 (`pnpm --filter app tsc --noEmit`)
