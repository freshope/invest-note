...
- [ ] Step 1 — `routers/accounts.py:37-38` SQL에 `WHERE user_id = $1` 추가, `user.id` 바인딩
- [ ] Step 1 — `pytest tests/test_accounts.py -q` 통과 + commit
- [ ] Step 2 — `routers/portfolio.py:90-101` 인라인 SQL 제거, `list_trades_with_account` import 및 호출
- [ ] Step 2 — `pytest tests/test_portfolio.py -q` 통과 + commit
- [ ] Step 3 — `db_ops/trades_repo.py` 에 `list_trades_in_group(conn, user_id, key: TradeGroupKey)` 신규 함수
- [ ] Step 3 — `routers/trades.py` POST(`:150`)/PATCH(`:257`)/DELETE(`:288`) 호출 교체
- [ ] Step 3 — `routers/trades.py` `import_commit`(`:466-560`) 그룹 partition 선분할 + 그룹별 fetch + recalc
- [ ] Step 3 — `pytest tests/test_trades.py tests/test_trades_import.py -q` 통과 + commit
- [ ] Step 4 — `external/quotes.py` `_inflight` 맵 추가, `_get_cached` 재작성
- [ ] Step 4 — single-flight 동작 검증 테스트 추가 (동일 키 동시 호출 시 fetch_fn 호출 횟수 = 1)
- [ ] Step 4 — `pytest tests/test_quotes.py -q` (또는 신규 테스트) 통과 + commit
- [ ] 전체 `cd api && poetry run pytest -q` 통과
- [ ] spec-current → spec-history 이동, backlog 후속 항목 추가
