...
- [ ] `docs/decisions.md` 에 E/F 통합 미진행 결정 기록 + `docs/backlog.md` E, F 두 줄 제거
- [ ] `aggregate.py` / `analysis.py` 에 `build_strategy_evaluations` 호출 의도 주석 + docstring 보강 (F1)
- [ ] `domain/portfolio.py` `Lot` frozen dataclass 추가 + `build_positions` walk 종료 시점에 1 회 생성 패턴으로 변경 (G1-1)
- [ ] `build_account_snapshots` 의 lot dict 접근 → attribute 접근 (G1-2)
- [ ] `domain/trade_import.py` `PreviewSignature` / `make_preview_signature` / `trade_to_preview_signature` 추가 (I1-1)
- [ ] `routers/trades.py` import / preview 경로를 `make_preview_signature` 로 전환하고 `_PREVIEW_ACCT` 삭제 (I1-2)
- [ ] `cd api && poetry run pytest -q` 전체 그린 확인
- [ ] 항목별 독립 커밋 (`docs:` / `refactor(analysis):` / `refactor(portfolio):` / `refactor(trade_import):`)
