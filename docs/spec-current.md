...
- [ ] **E.** Literal 타입 명시 (`UntaggedLiteral`, `EmotionBucket`, `ReasoningTagBucket`) + 사용처 타입 좁힘
- [ ] **D.** `_decimal_to_*` 3 validator 공통 헬퍼 추출
- [ ] **C.** `_first_bucket_label` 일반 헬퍼 + `Counter` 기반 dist 빌드
- [ ] **B.** trade_import signature 4함수 통합 + `parse_kst_date` 헬퍼로 trades.py KST 파싱 흡수
- [ ] **A.** `accounts_repo.list_accounts` 헬퍼 + 라우터 3곳 인라인 SELECT 흡수
- [ ] **F.** `decisions.md` 기록 + `backlog.md` 의 `_parse_*` 항목 제거
- [ ] 백엔드 pytest 통과 (`cd api && poetry run pytest -q`)
