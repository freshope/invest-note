...
- [ ] BE: `SellBreakdown` dataclass / `compute_flexible_breakdown` / `SellBreakdownResponse` 에서 `is_manual_input` 제거
- [ ] BE 테스트: `is_manual_input` 참조 grep 후 정리, `cd api && poetry run pytest -q` 통과
- [ ] FE: `holdings.ts` 의 `SellBreakdown` 타입과 `computeFlexibleBreakdown` 에서 `isManualInput` 제거
- [ ] FE: `TradeDetail.tsx:177` 의 `!summary.breakdown.isManualInput` 분기 제거
- [ ] FE 테스트: `holdings.test.ts:48` 의 `isManualInput` 단언 제거
- [ ] FE: `query-keys.ts` 의 `accounts` 를 `["portfolio", "accounts"]` 로 변경
- [ ] FE: `AccountCard.tsx` / `AccountFormPanel.tsx` 의 mutation 후 `invalidateQueries(accounts)` 호출 제거 + `Promise.all` 단순화
- [ ] `pnpm tsc --noEmit` 통과 (`pnpm -C app exec tsc --noEmit`)
- [ ] `pnpm test` 통과 (`pnpm -C app test`)
- [ ] `docs/backlog.md` 의 두 항목에 처리 완료 메모 추가
