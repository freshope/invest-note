...
- **rate limit 공유 한도 — 실측 2건/초(2026-06-07, 개인 실전 계정)**: 사전 조사의 ~20req/s 는 법인/과거 수치로 보임. 3번째 연속 호출부터 EGW00201. 대응: `kis.py` 슬라이딩 윈도우 페이싱(2건/1.05s, 토큰 발급 포함) + EGW00201 1회 재시도. 시세 경로는 슬롯 대기 예산 1.0s — 초과 시 즉시 naver fallback(전체 deadline 보호).
  - **⚠️ 리미터는 per-process — "단일 인스턴스 한정" 해결**: 2건/초는 appkey 당(계정 전역) 서버 제한인데 리미터·토큰 캐시는 프로세스 단위다. 컨테이너당 uvicorn 1프로세스(Dockerfile 확인)지만 Coolify replica 가 N 이면 합산 2N/초가 나가 초과분은 여전히 EGW00201(fallback 으로 흡수는 됨). **kis 공급자 활성화(env 전환) 선행조건: replica=1 확인 또는 공유 리미터(Redis 등) 도입** — 백로그 "Preview staging 멀티 워커 대응"과 같은 계열.
  - **함의: KIS 를 시세 1차 공급자로 두면 멀티 종목 화면에서 대부분 naver 로 흐른다**(per-process budget + replica 간 경쟁) — 시세는 보조 공급자로 보거나, 한도 상향(법인 제휴) 후 1차 전환 검토.
...
- **crossvalidate=kis 의 KONEX 박제**: 마스터 파일엔 KONEX 가 없는데 배치 전원을 checked 로 마킹 → KONEX 종목이 "검증됨"으로 기록돼 이후 naver 로 되돌려도 재검증되지 않음(`naver_checked_at` 공유 컬럼). kis 로 전환 시 인지할 것.
- **KIS 시세의 traded_on=None**: 휴장일 판정 신호가 KIS 경로에선 항상 None (naver basic 과 동일한 degrade — 처리 가능한 기존 경로).
