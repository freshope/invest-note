...
## 완료 조건 (검수)
- [ ] C1~C5 `pnpm -C app exec tsc --noEmit` 통과 + 동작 시나리오
- [ ] item 5 hint 자동기입 오염 방지 게이트(null-only·commit 시점·mismatch 경고·invalidate·name 동봉) 반영
- [ ] 미커밋 F1~F5 리뷰 수정 이관(삭제 아님) 확인
- [ ] `docs/decisions.md` 갱신(C6)
- [ ] BE 변경 없음 확인(passthrough/마이그레이션 기존 사용)
