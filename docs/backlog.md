...
- [ ] **푸시 전송 활성화 (게이트, 2026-07-28 갱신)** — 전송 경로는 Firebase Admin 단일 채널로 재작성 완료(`services/push/`, `device_tokens`(0020), `POST /me/push-token`, FE `@capacitor-firebase/messaging`, iOS `GoogleService-Info.plist` 타깃 등록)이나 **자격증명 없으면 no-op**. 남은 활성화 전제: ① Coolify 에 `GOOGLE_APPLICATION_CREDENTIALS_JSON`(한 줄 JSON) 주입 + 구 `FCM_SERVICE_ACCOUNT_JSON`/`APNS_*` 제거, ② Firebase Console `invest-note-494103` → Cloud Messaging 에 APNs `.p8` 업로드(없으면 iOS 는 토큰만 받고 알림 미도착), ③ 실기기 검증 Android/iOS(권한 허용→토큰 등록→어드민 답변→실제 수신→탭 딥링크). `aps-environment` production 승격은 **불필요**해짐(Firebase 가 자동 라우팅). ⚠️ 네이티브 플러그인 교체라 릴리즈 scope web-only **오판 금지**(feedback_release_scope_native_plugin_gap) — 네이티브 릴리즈 + 양 스토어 재심사. 활성화 경로는 자동 테스트 커버리지 없음. 참조: `docs/decisions.md` 2026-07-28.
