...
- [ ] **어드민 BE-auth 배포 + 코드리뷰 후속 (2026-06-26)** — 어드민 패널 BE 토큰-브로커 전환(`feature/admin-be-auth` → develop) 완료, **운영 배포 대기**. 배포는 hard-swap(롤백 플래그 없음): **(선행)** Coolify api 에 `BE_ADMIN_REDIRECT_URL`(=`https://invest-note-admin.pixelwave.app/auth/callback/`) 주입 + `CORS_ORIGINS` 에 어드민 origin 포함 확인(SSOT) → **(필수 게이트)** 실제 OAuth 로그인 1회 수동 e2e(실패 시 어드민 lockout) → 어드민 SPA 배포. ⚠️ **로컬 e2e** 는 Google(/Kakao/Apple) OAuth 클라이언트에 `http://localhost:3108/auth/callback` 등록 필요(BE↔IdP redirect, 네이티브와 공유 — 운영은 등록됨). **스킵된 코드리뷰 후속(저severity, 내부 콘솔)**: #3 `_handle_callback` 실패 레그가 web 어드민에 raw JSON 노출(만료 state 는 client 식별 불가라 근본 수정 한계) / #4 `signInWithGoogle` full-page 이동이라 dormant-503 catch 불가(env 미설정 시에만, 배포 체크리스트가 커버). 트리거: 2c 착수 전 어드민 배포 시.
...
- [ ] 공지사항 페이지네이션 (2026-06-26 설정 게시판 후속) — `NoticePanel`(`app/src/components/settings/NoticePanel.tsx`)이 `boardApi.listNotices()` 1페이지(최근 20건)만 로드하고 "더 보기"가 없다. BE `GET /v1/board/notices` 는 `page`/`page_size`·`total` 을 이미 반환하므로 FE 에 `useInfiniteQuery` 무한스크롤 또는 더보기 버튼만 추가하면 된다. 트리거: 공지 누적으로 20건 초과 시.
