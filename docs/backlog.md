...
- [ ] **eslint lint warnings 잔여 (2026-06-30, set-state-in-effect 라운드 후속)** — admin eslint config 부재(전역 lint 실패)는 2026-06-30 해소(`admin/eslint.config.mjs` 추가 + app/admin `react-hooks/set-state-in-effect` 에러 5+1건 정식 수정: 대부분 렌더 중 상태조정 패턴 전환, object-URL previews·딥링크 consume 2건만 사유주석 disable). **남은 것:** app `pnpm -C app lint` 의 advisory warnings 7건 — ① `<img>`→`next/image`(`BrokerLogo.tsx`·`trade-display.tsx`, 시각/레이아웃 영향 검토 필요), ② 테스트의 `_`-prefix 미사용 var(`CapacitorDeepLinkHandler.test`·`lib/auth/index.test`, config `argsIgnorePattern:^_` 로 일괄 무시 가능), ③ `compilation-skipped`(React Compiler "incompatible library" — `TradeEditPanel.tsx`·`AccountFormPanel.tsx`). 에러 아님(exit 0). 트리거: 코드 품질 라운드 또는 CI lint 게이트 `--max-warnings 0` 도입 시.
