...
- [ ] **내 자산 변화 페이지 추가** — 선행 조사 필요로 보류(2026-06-03). 요구사항:
  - 계좌별 자산 / 종목별 자산의 일별 변화를 차트로 표시
  - 차트 아래에 목록 표시
  - 계좌 필터 추가
  - 차트는 다른 차트와 마찬가지로 컨트롤/포커스 제거
  - 메인 헤더 우측에 아이콘 추가 → 계좌별 자산 변화 페이지 진입
  - 종목 상세 종목명 우측에 아이콘 추가 → 종목별 자산 변화 페이지 진입
  - 트리거: 선행 조사 완료 후 spec 작성·구현 착수
  - 일별/종목별 종가를 테이블에 저장 (2년치 데이터 유지)
  - 페이지 진입시 종가 데이터를 테이블에서 확인 > 데이터가 없거나 누락된 날짜가 있는 경우 data.go.kr api를 사용해 테이블에 적재 후 사용
  - 당일 데이터는 현재가 사용
  - 차트는 스케일 변화는 안되고 좌/우로 이동하여 과거 날짜를 확인 가능하개 구현 (최대 2년)
