...
- [ ] **탈-Supabase Auth Phase 2 — BE token-broker (2026-06-19)** — Phase 1(결합 격리, `docs/spec-history/2026-06-19-auth-decoupling-phase1.md`·`decisions.md` 2026-06-19) 완료. Phase 2 = BE 가 OAuth 를 중개하고 **자체 토큰 발급** → 앱은 IdP SDK 를 전혀 안 가짐. 그래야 IdP 교체가 BE 배포만으로 끝남(앱 스토어 심사/OTA 불필요 — "API 중심"의 진짜 동기). **무거운 난점:** ① refresh 토큰 관리의 BE 이관(현재 supabase-js 자동 갱신), ② 딥링크에 토큰 직접 노출 금지 → 일회용 code 핸드오프, ③ Apple Sign In 네이티브 경로가 순수 브라우저 플로우와 충돌 가능, ④ BE 토큰 서명키 관리·만료·로테이션 신규 도입. **⚠️ 쿠키 세션 BFF 는 불가**(Capacitor `capacitor://localhost` 쿠키 미저장) → bearer token-broker 모델이어야 함. 조사·설계 전문: `_workspace/auth-decoupling-research.md`. 트리거: IdP 실제 교체 결정 시 또는 봇 어뷰징(public key 노출, [[project_supabase_oauth_only]]) 대응 필요 시.
