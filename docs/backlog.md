...
- [x] ~~공지사항 페이지네이션~~ — 2026-06-30 완료(`NoticePanel` `useInfiniteQuery` + "더 보기", queryKey `noticesList` 분리). **후속 풀스택 통합 진행 중**(아래): my-posts(의견/오류/제보)도 동일 적용 위해 게시판 공통 API(board_type 필터+페이지네이션)·unread 전용 API 신설·FE 공통 컴포넌트로 통합 — 이때 notices 도 공통 컴포넌트로 흡수 검토.
