...
- [ ] **탈-Supabase Auth Phase 2 — BE token-broker 코드 완료, 출시/활성화·2c 대기 (2026-06-19)** — **코드 구현 완료**(2a expand 토대 dormant / 2b-1 BE OAuth 중개·토큰 발급·refresh·profile / 2b-2 FE 네이티브 BE flow 전환). 사양: `decisions.md` 2026-06-19 (2a/2b-1/2b-2), 설계 `_workspace/auth-phase2-design.md`. 머지는 **dormant-safe**(BE env 미주입 시 발급 off·Supabase 무회귀, FE 503 우아 처리). **남은 것:**
  ...
  - **② 운영 활성화(2b)**: BE env(provider client secret·Apple `.p8`·BE ES256 서명키·`be_token_audience`·`be_oauth_redirect_base`) + IdP 콘솔 redirect_uri 에 BE callback 추가(Supabase 와 둘 다) + 마이그레이션 **0004+0005+0006** 적용 + identity 적재·profile 백필(dry-run 선행). + A1 운영 토큰 iss 실측(prod 핀 활성화 시 — 단 Supabase fallback 이라 핀은 선택).
  - **③ 배포**: BE Coolify(dormant 먼저 가능) / FE 스토어 빌드(secure storage 네이티브 플러그인 → **OTA 불가**).
  ...
  - **⑤ DEFER(코드리뷰 후속)**: AuthStrategy 인터페이스로 isNativePlatform 이중화 일반화(2c 가 웹 분기 삭제하므로 그때) / BE `/auth/logout` refresh-revoke 엔드포인트(로그아웃이 서버측 30d refresh 안 죽임 — 탈취 기기 강제 로그아웃) / 딥링크 scheme 상수↔BE `BE_DEEPLINK_SCHEME` 정합 주석 / `getAccessToken` cold-start 캐시 미스 race(저severity, 최대 1h stale·30d 부활은 가드됨).
...
- [ ] **admin eslint 설정 부재 — `pnpm lint` 전역 실패 (2026-06-18 발견)** — `admin/package.json` 의 `lint` 스크립트가 bare `eslint` 인데 `admin/` 에 `eslint.config.js`(flat config, ESLint v9 요구)가 없어 lint 가 프로젝트 전역에서 실패한다(누적 사용자 차트 작업 중 발견, 기능과 무관). `eslint-config-next@16.2.3`·`eslint@^9` 는 설치돼 있으므로 `eslint.config.mjs` 에 `next/core-web-vitals` flat config 추가하면 해소. app(`app/`)도 동일 점검 필요. 트리거: admin CI lint 게이트 도입 또는 코드 품질 라운드.
...
- [ ] **FORCE RLS Phase 3 — `app_authenticated` 역할 제거 (2026-06-18 예정)** — Phase 1(035 FORCE)·Phase 2(`acquire_for_user` SET ROLE 제거, api-v1.3.0)는 2026-06-17 운영 적용·검증 완료(forcerls 3개 t·owner+FORCE fail-closed 0행·앱 종단 두 계정 격리 정상). 남은 정리: ① `supabase/migrations/036_drop_app_authenticated.sql`(이미 작성, 미커밋) 적용 = `drop owned by app_authenticated; drop role app_authenticated;`, ② `api/src/invest_note_api/auth/constants.py` 의 미사용 `DB_APP_ROLE` 상수 제거 + 커밋. **선행 조건: Phase 2 운영 베이킹(반나절~하루) 무이상 확인** — BE 로그에 RLS 에러·0행 민원 없을 것. ⚠️ **비가역**: 적용 순간 구 BE(SET ROLE 방식) 롤백 경로 소멸. invest_note_app 은 테이블 owner라 이 grant 불필요(무영향). prod 적용은 superuser 필요 → `docker exec psql -U postgres`(운영 DB 호스트 포트 미publish — 접근법은 memory `project_prod_db_access`). dev 도 parity 위해 동일 적용. → **(2026-06-17, `feature/alembic-migrations`) Alembic 도입과 연동**: 036 을 prod/dev 에 적용한 **직후** 라이브 스키마 pg_dump 스냅샷을 떠 `0001_baseline`(role-free clean baseline)으로 만든다. 그래서 036 은 별도 Alembic 리비전이 아니라 baseline 에 흡수됨. 이후 `supabase/migrations/*.sql` 은 `migrations_archive/` 로 이동. **진행(2026-06-17, 미커밋):** dev :64340 에 036 적용 + `DB_APP_ROLE` 상수 제거 + `0001_baseline` 생성·dev stamp·migrations_archive 이동 완료. **prod 적용 완료(2026-06-17):** prod 에 036 적용(⚠️ `drop owned by` 는 per-DB라 반드시 `invest_note` DB 컨텍스트에서 — 다른 DB 접속 시 `drop role` 이 "objects depend on it" 로 막힘) + drift diff 확인(타입/함수 owner 무해 차이만, decisions ⑤) + `alembic_version` stamp. dev·prod 모두 `0001_baseline`.
