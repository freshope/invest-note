...
- [ ] **푸시 `Message.token` → `fid` 전환 대응 (잠재 부채, 2026-07-28)** — firebase-admin 7.x 가 `Message.token` 을 deprecate 하고 `fid`(Firebase Installation ID)를 권한다. 현재는 `token` 유지가 **정답** — FE `@capacitor-firebase/messaging` 은 `getToken()`(등록 토큰)만 노출하고 installation ID 를 주지 않아, fid 로 바꾸면 타깃이 달라져 전송이 깨진다(`services/push/firebase.py` 에 주석). 트리거: firebase-admin 이 `token` 을 **실제로 제거**하면 → `@capacitor-firebase/installations` 플러그인 추가 + FE 등록 경로 변경 + `device_tokens` 저장값 의미 변경 + **네이티브 재심사**가 한 묶음으로 필요하다. 그때까지는 major 업그레이드 시 릴리즈 노트만 확인.
