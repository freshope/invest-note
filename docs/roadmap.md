...
| 스타일링 | Tailwind CSS |
| 백엔드 | FastAPI (Python) — OIDC issuer registry verifier(BE 자체 토큰 ES256, 2c에서 Supabase fallback 제거) + Authlib OAuth 중개 + asyncpg (사용자 격리=앱 레이어 user_id 필터) |
| DB/Auth | self-hosted PostgreSQL (Coolify) + Alembic 마이그레이션 (RLS 제거, 2026-06-18) / Auth는 **BE token-broker 전환 완료** — Phase 1 결합 격리 + Phase 2(BE OAuth 중개·자체 토큰 발급·refresh·profile / FE 네이티브 BE flow / 2b-3 신규가입 경로 / 2b-4 BE flow 서버 플래그) + **운영 cutover(2026-06-26)** + 어드민 BE flow(v1.3.2) + **2c 최종 teardown(2026-07-01) — 런타임 Supabase 결합 0**(회원탈퇴 DB-only, config·디렉토리 잔재 제거). 남은 것은 비가역 운영(Coolify SUPABASE env 제거·클라우드 프로젝트 삭제·PIPA 고지)뿐 |
...
- AI 분석
- 푸시 알림 (v2.5 모바일앱에서) — 🚧 알림 이력(Phase 1) 출시 가능 · 전송(Phase 2) 코드 완료·활성화 게이트(시크릿·iOS production 승격 대기, `docs/spec-history/2026-07-22-board-notifications.md`)
- 세금 계산 자동화
