...
- [x] `app/` 디렉터리 생성 + `git mv` 로 Next.js 자산 일괄 이동 (src, public, scripts, next.config.ts, tsconfig.json, components.json, eslint.config.mjs, postcss.config.mjs, vitest.config.ts, package.json)
- [x] gitignored 파일 수동 이동: `.env.local`, `.env.local.example`, `next-env.d.ts` → `app/`
- [x] `.next/`, `tsconfig.tsbuildinfo` 삭제
- [x] `api/` 디렉터리 생성 + `api/README.md` 작성
- [x] `pnpm-workspace.yaml` 수정 (`packages: ["app"]` 추가)
- [x] 루트 `package.json` 신규 생성 (위임 스크립트만)
- [x] `.gitignore` 경로 보정 + `app/.gitignore` 신규 생성
- [x] `README.md` 디렉터리 구조 섹션 추가
- [x] `pnpm install` (루트) 정상 동작 확인
- [x] `pnpm dev` (루트) — Next.js 개발 서버 정상 기동 확인
- [x] `pnpm build` (루트) — 빌드 통과
- [x] `pnpm tsc` (루트) — 타입 체크 통과
- [x] `pnpm lint` (루트) — 린트 통과
- [x] `pnpm test` (루트) — vitest 통과
- [x] `git log --follow app/src/app/login/page.tsx` — 이전 히스토리 보존 확인
