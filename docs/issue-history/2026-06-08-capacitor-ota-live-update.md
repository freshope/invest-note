...
- [x] 모든 코드 단위 verify 통과(`pytest` 529 passed / `tsc` exit 0 / `cap sync` 성공 / zip 드라이런).
- [x] BE no-update 응답 = `200 {"kind":"up_to_date"}` (빈 200/`{}`/204 금지) — 결정 1(2026-06-08 실측 정정) 준수.
- [x] `required_native_version` 과 `min_supported_version` 분리 유지 — 결정 3.
- [x] R2 토큰 fe public env 미누출 — 결정 4.
- [x] `docs/decisions.md` 갱신.
- [x] **(MERGE-BLOCKING)** be-engineer 실측 노트 3건(필드 케이스 / no-update 동작 / `version_build` vs `version_code` 의미) 회신 — `version_build`=마케팅 버전 확정, no-update=`{"kind":"up_to_date"}` 확정. `_workspace/03_fe_changes.md` 참조.
- [x] `version_name=="builtin"` 신규설치 중복 다운로드 없음(단위 2 pytest).
- [x] **v1 경계 명시**: 라이브 OTA·실기기 스큐 매트릭스는 스토어 제출(단위 0) 이후 후속.
- [x] spec → `docs/issue-history/2026-06-08-capacitor-ota-live-update.md` 이동.
