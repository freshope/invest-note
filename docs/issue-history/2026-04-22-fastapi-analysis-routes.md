...
- [x] `domain/analysis/__init__.py` + `period.py` (parse_period, filter_by_period KST)
- [x] `domain/analysis/holding_period.py` (compute_holding_days_map)
- [x] `domain/analysis/concentration.py` (HHI 상수 + compute_concentration)
- [x] `domain/analysis/aggregate.py` (dataclass + compute_summary + tag 귀속 로직)
- [x] `domain/analysis/profile.py` (compute_profile + input_rates)
- [x] `domain/analysis/rules.py` (10 규칙 + evaluate_rules 정렬)
- [x] `tests/test_analysis_logic.py` (각 순수 함수 단위 테스트, 경계값/빈 거래 포함)
- [x] `routers/analysis.py` (3 endpoint + bucket 상수/함수)
- [x] `main.py` 라우터 등록
- [x] `tests/test_analysis.py` (정상/period 필터/시세 실패 fallback/401)
- [x] `api/README.md` curl 예시 갱신
- [x] `poetry run pytest` 전체 통과
- [x] `poetry run ruff check` 통과
- [x] 로컬 uvicorn + curl 3 endpoint 검증
