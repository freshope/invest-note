...
- [x] `docs/backlog.md` — 2단계 서브 항목 갱신(완료 표시 + 남은 chunk 명시).
- [x] `app/.env.local.example` — `NEXT_PUBLIC_API_BASE_URL=` 추가.
- [x] `app/src/lib/api-client.ts` — `apiFetch`에 base URL + Bearer 주입, `portfolioApi`/`stocksApi`/`analysisApi` 래퍼 추가.
- [x] `app/src/hooks/usePortfolioSummary.ts` — `portfolioApi.summary()` 사용.
- [x] `app/src/hooks/useAnalysisData.ts` — `analysisApi.summary/behavior/suggestions()` 사용.
- [x] `app/src/components/records/StockSearchInput.tsx` — `stocksApi.search()` 사용.
- [x] `app/src/components/records/TradeBasicForm.tsx` — `portfolioApi.holding()` 사용.
- [x] 타입 체크 통과 (`pnpm --filter app tsc --noEmit` 또는 `pnpm --filter app lint`).
- [x] 로컬 E2E 수동 검증 — FastAPI(:8000) + Next.js(:3000) 동시 구동, 홈/기록/분석/설정/거래 등록 흐름 확인, Network 탭에서 `localhost:8000/api/*`로 요청 + `Authorization: Bearer` 헤더 확인.
- [x] 롤백 스위치 확인 — `.env.local`에서 `NEXT_PUBLIC_API_BASE_URL` 제거 후 기존 `/api/*`로 fallback 되는지 확인.
