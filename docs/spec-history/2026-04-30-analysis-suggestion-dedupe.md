...
- [x] `AnalysisDashboard.tsx` 섹션 2를 `SectionCard` + `SuggestionList`로 교체하고 `suggestionsData &&` 가드 추가
- [x] `AnalysisDashboard.tsx` 섹션 10 (기존 "투자 방향성 제안" SectionCard) 제거
- [x] `AnalysisDashboard.tsx`에서 `InsightHighlights` import 및 `InsightSection` 인라인 함수, 미사용된 `SuggestionsData` 타입 import 제거
- [x] `app/src/components/analysis/InsightHighlights.tsx` 파일 삭제
