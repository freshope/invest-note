...
> 완료: 2026-06-20 (코드. 디바이스 스모크·운영 활성화·2c 는 backlog 후속)
...
- [x] B12 enforce-always 정정 반영(spec/decisions, C1)
- [x] F-1 secure storage 플러그인(Capacitor8 호환·Keychain/Keystore) 추가 + OTA 불가 명시
- [x] F-2 토큰 store(secure, verifier 영속 C2/C5) + F-3 PKCE S256(C1) + F-4 BE fetch/디코드(C10)
- [x] F-5 lib/auth 7함수 네이티브 BE flow 분기 — **single-flight refresh(C3)·실패 logout(C4)·웹 무회귀(C8)**
- [x] F-6 딥링크 code→/auth/token 단일(implicit fragment 제거 C6)·503 우아(C7)·cold-start verifier(C2)
- [x] F-7 login BE flow / F-8 AuthProvider logout emit / F-9 signOut store clear(C11)
- [x] F-10 supabase-js 제거 = **2c assess 결론**(2b-2 제거 아님)
- [x] `pnpm -C app exec tsc --noEmit` + `pnpm -C app test` 전체 무회귀
- [x] grep 불변식: localStorage 토큰 평문 부재(C5) / 네이티브 access_token fragment 파싱 부재(C6) / supabase-js 1파일(F-10)
- [x] `docs/decisions.md` 갱신(D-2b2)
- [x] spec → `spec-history/2026-06-19-auth-decoupling-phase2b2.md` 이동 준비
