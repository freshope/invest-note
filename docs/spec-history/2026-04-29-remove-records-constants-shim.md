...
- [x] `AutoMetaField.tsx` import 경로 교체
- [x] `StrategyEmotionFields.tsx` import 경로 교체
- [x] `TradeMetaBuyForm.tsx` import 경로 교체
- [x] `TradeEditPanel.tsx` import 경로 교체
- [x] `TradeDetail.tsx` import 경로 교체
- [x] `app/src/components/records/constants.ts` 삭제
- [x] 타입 체크 통과 (`pnpm tsc`)
- [x] 테스트 통과 (`pnpm -C app test`)
- [x] 잔존 참조 없음 검증 (`grep -r "components/records/constants" app/src`, `grep -rE "from ['\"]\\./constants['\"]" app/src/components/records`)
