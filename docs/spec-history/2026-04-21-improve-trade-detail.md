...
- [x] `src/types/database.ts`의 `Trade` 인터페이스에 `exchange` 필드 추가
- [x] `src/components/records/TradeBasicForm.tsx` zod 스키마에 `exchange` 추가, `onSelect`에서 `setValue`, create payload 포함
- [x] `src/components/records/TradeDetail.tsx` 종목 헤더 카드에 단가·수량·총액 블록 추가
- [x] `src/components/records/TradeDetail.tsx` 기본 정보 박스를 2열 컴팩트 그리드로 변경 + 시장+거래소 표기 로직 적용
