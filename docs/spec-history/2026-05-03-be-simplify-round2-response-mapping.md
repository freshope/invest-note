...
- [x] `db_ops/accounts_repo.py` — `account_row_to_dict` 에 UUID 필드 변환 추가 (`id`, `user_id`)
- [x] `db_ops/accounts_repo.py` — `UPDATABLE_COLS` / `RETURNING_COLS` 상수 + `patch_account(conn, account_id, patch) -> dict | None` 신규
- [x] `routers/portfolio.py` — `_account_from_row` 의 UUID 변환 for-loop 삭제
- [x] `routers/accounts.py` — `update_account` 가 `patch_account` 호출로 위임
- [x] `routers/analysis.py` — `summary` 손-매핑 13줄 → `{"period": period_val, **asdict(summary)}`
- [x] `routers/trades.py` — `create_trade` 의 `Trade(...)` 17줄을 `**data.model_dump()` spread 로 단순화
- [x] 테스트 통과: `cd api && poetry run pytest -q` → 251 passed
- [x] `docs/decisions.md` 에 Item 3 (`_trade_with_account_dict` → `TradeWithAccountResponse`) 미진행 결정 기록
- [x] `docs/backlog.md` "응답 매핑 / 라우터 청소" 섹션에서 Item 3 줄 제거
