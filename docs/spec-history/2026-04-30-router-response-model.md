...
- [x] `schemas/_base.py` — `CamelModel`, `to_camel_pnl` 정의
- [x] `schemas/portfolio_response.py` — Position/AccountSnake/AccountSnapshot/DashboardTotals/PortfolioSummary 응답 모델
- [x] `routers/portfolio.py` — `_pos_dict`/`_snap_dict`/`_totals_dict` 제거, `/summary`에 `response_model` 적용
- [x] `schemas/analysis_response.py` — Stats×4 / Summary / Behavior / Suggestions 응답 모델
- [x] `routers/analysis.py` — 3개 라우트 `response_model` 적용 (`/suggestions`은 `exclude_none`)
- [x] `schemas/trade_response.py` — Breakdown/StrategyEvaluation/TradeSummary 응답 모델
- [x] `routers/trades.py` — `_breakdown_dict` 제거, `/{trade_id}/summary`에 `response_model` + `exclude_none`
- [x] `tests/test_portfolio.py` — PnL 키 및 snake_case account 검증 추가
- [x] `tests/test_analysis.py` — `sumPnL` 키 검증 추가
- [x] 백엔드 테스트 통과 (`cd api && poetry run pytest -q`)
- [x] 프론트엔드 타입 체크 (`pnpm tsc`) — 변경 없지만 회귀 미발생 확인
