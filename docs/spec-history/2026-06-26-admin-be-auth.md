...
> 완료: 2026-06-26 (구현·QA·코드리뷰 완료 / ⚠️ 실제 OAuth 수동 e2e + 운영 배포는 게이트로 이월 — backlog "어드민 BE-auth 배포")
...
- [x] 작업 1~9 모든 verify 통과(`poetry run pytest` 25 passed / `pnpm -C admin exec tsc --noEmit` / app build)
- [x] QA 3·10 통과 + 11 shape·계약 정합 통과(drift 0)
- [ ] **실제 OAuth 로그인 수동 e2e** — 배포 게이트로 이월(hard-swap, 미완). 로컬은 Google 콘솔에 `http://localhost:3108/auth/callback` 등록 필요
- [x] `grep -rn "supabase" admin/src` == 0
- [x] `docs/decisions.md` 갱신(작업 12)
- [x] 배포 순서 노트 사용자 전달(BE env + CORS 선행)
- [x] spec → `docs/spec-history/2026-06-26-admin-be-auth.md` 이동 준비
- [+] 코드리뷰 --fix 5건 반영 + `/auth/logout` revoke 추가(app+admin) + app zod override 픽스
