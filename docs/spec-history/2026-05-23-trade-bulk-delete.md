...
- [x] BE: `TradeBulkDeleteRequest` 스키마 (1~200건)
- [x] BE: `POST /trades/bulk-delete` 라우터 — 트랜잭션·정렬 락·일괄 검증·일괄 삭제·그룹별 recalc
- [x] BE: pytest — 여러 그룹 혼합 성공, oversell 충돌, 누락 id
- [x] FE: shadcn checkbox 설치 + base 래퍼
- [x] FE: `useLongPress` 훅
- [x] FE: `useTradeSelection` 훅
- [x] FE: `TradeCard` 선택 props + 체크박스 UI
- [x] FE: `TradeList` 선택 모드 헤더·전체 선택 토글·FAB/업로드 숨김·필터 변경 시 초기화
- [x] FE: `tradesApi.bulkDelete`
- [x] FE: 확인 다이얼로그 + mutation + invalidate + 토스트
- [x] FE: vitest — `useTradeSelection`, `TradeList` 선택 모드
- [x] 타입 체크 통과 (`pnpm -C fe exec tsc --noEmit`), BE 테스트 통과 (`cd be && poetry run pytest -q`)
