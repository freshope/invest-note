...
- [x] `domain/holdings.py` — import 정리 (`TRADE_TYPE_BUY` 제거, `WalkerState`/`walk_trades` 추가)
- [x] `domain/holdings.py:36` — `compute_holding_summary` 본문을 walker terminal state 기반으로 재구성
- [x] `domain/portfolio.py` — `_build_lot_map(trades) -> LotMap` 헬퍼 추출 (lines 110-162 의 Phase 1)
- [x] `domain/portfolio.py` — `_lot_to_positions(lot_map) -> list[Position]` 헬퍼 추출 (lines 164-219 의 Phase 2)
- [x] `domain/portfolio.py` — `build_positions` 본문을 두 헬퍼 합성 (5 줄) 로 단순화, docstring 보존
