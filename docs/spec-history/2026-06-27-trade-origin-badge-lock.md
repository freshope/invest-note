...
>
> 구현 시 계획 대비 조정: (1) 타입 `origin`은 **required**로 구현(FE 픽스처에 origin 추가). (2) 배지 색상 중립 → **그린 톤**(import 성공색 일관). (3) 안내문은 가격 입력 위 **Alert 컴포넌트**(base/Alert 신규). (4) baseline_schema.sql에는 origin 미추가 — **0007 마이그레이션에만**(코드리뷰: 둘 다면 신규 DB DuplicateColumn).
...
- [x] 모든 단위 verify 통과 (BE pytest 868 passed / FE tsc 그린 + 265 passed / 동작 시나리오)
- [x] 마이그레이션 로컬 적용·검증 (운영 DB 미적용 — 배포 시 0007 적용 필요)
- [x] BE 잠금 5필드 ↔ FE omit 5필드 정확 일치 (QA 교차 확인)
- [x] `docs/decisions.md` 갱신 — 2026-06-27 거래 origin 도입+금액 잠금 결정 기록
- [x] spec → `docs/spec-history/2026-06-27-trade-origin-badge-lock.md` 이동
