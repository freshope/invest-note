...
- [x] 모든 단위 verify 통과 (B1~B6, F1~F10)
- [x] BE 전체 `poetry run pytest -q` 무회귀 + iss 토글 3케이스 통과
- [x] FE 전체 `tsc --noEmit` + `pnpm -C app test` 무회귀
- [x] grep 불변식: BE `decode_supabase_jwt` 0건 / FE `@supabase/supabase-js` 1파일·`@/lib/supabase/client` 0건
- [x] `docs/decisions.md` 갱신 (D1)
- [x] 동작 변경 0 확인 (T2~T5, T7)
- [x] spec → spec-history 이동 준비 (Z1)
