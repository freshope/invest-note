...
- [x] `components/shared/EmptyCard.tsx` 신규 생성
- [x] `home/EmptyState.tsx` 를 `EmptyCard` 위로 재구성
- [x] `records/TradeList.tsx` 인라인 빈 상태 2건 → `EmptyCard` 치환
- [x] `settings/AccountList.tsx` 인라인 빈 상태 → `EmptyCard` 치환
- [x] `stocks/StockDetail.tsx` 인라인 빈 상태 → `EmptyCard compact` 치환
- [x] `docs/backlog.md` Round 7 처리 메모
- [x] 타입 체크 (`pnpm tsc`)
- [x] FE 단위 테스트 (`pnpm -C app test`)
