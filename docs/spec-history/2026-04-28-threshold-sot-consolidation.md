...
- [x] `thresholds.py`에 도메인 판정 임계값 6개 + 샘플 가드 8개 추가
- [x] `rules.py` import 확장 + 9개 규칙 함수의 비교문 매직 넘버 → 상수 치환
- [x] `SummaryCards.tsx` 하드코드 60/40 → `WIN_THRESHOLD`/`LOSS_THRESHOLD`
- [x] `AnalysisDashboard.tsx` `evaluateRules` import 및 fallback 호출 제거
- [x] `lib/analysis/rules.ts`에서 `evaluateRules`/`_rule*`/임계값 import 삭제, 타입만 보존
- [x] `lib/analysis/strategy-adherence.ts` 함수 삭제 (타입은 위치 결정 후 처리)
- [x] `__tests__/analysis.test.ts`의 `evaluateRules` describe 블록 삭제
- [x] `docs/decisions.md`에 후속 결정 항목 추가
- [x] BE 테스트 통과 (`cd api && poetry run pytest -q`)
- [x] FE 타입 체크 통과 (`pnpm -C app exec tsc --noEmit`)
- [x] FE 테스트 통과 (`pnpm -C app test`)
