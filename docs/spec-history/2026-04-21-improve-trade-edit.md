...
- [x] `src/components/records/trade-display.tsx` 생성 — `MARKET_LABELS`, `buildMarketDisplay(trade)`, `CompactRow`, `CountryBadge` export
- [x] `src/components/records/TradeDetail.tsx` 공용 모듈 사용으로 교체 (렌더 동일)
- [x] `src/components/records/TradeEditPanel.tsx` 상단부 교체 — `거래 유형` 박스 + `종목`/`날짜`/`계좌` ReadOnly 3개 제거 → 헤더 카드 + 2열 compact grid
- [x] `TradeEditPanel`에서 `watch`로 실시간 총액 계산 후 헤더 카드에 주입
- [x] 미사용 `ReadOnlyField` 헬퍼 제거
- [x] 타입 체크 통과 (`pnpm tsc --noEmit`)
- [x] 수동 검증: 매수/매도, KOSPI/NASDAQ/exchange null 거래, 가격·수량 실시간 반영
