...
Phase 1:
- [x] 단계 1~16 모든 verify 통과 (`poetry run pytest -q`, `pnpm -C app exec tsc --noEmit`,
      `pnpm -C app test`, E2E 시나리오)
- [x] `docs/decisions.md` 갱신(단계 15)
- [x] spec → `docs/spec-history/2026-07-22-board-notifications.md` 이동 준비

Phase 2(게이트):
- [x] 단계 17~20 코드 작성 + verify(no-op 경로)
- [ ] 단계 21 은 사용자 시크릿·iOS production 승격·실기기 검증 후 활성화 (backlog "Phase 2 푸시 전송 활성화" 로 이관)

> 후속(spec-finish 이후 확정): 알림→상세 딥링크를 목록 우회 대신 `GET /board/my-posts/{id}` by-id
> 엔드포인트로 구현(단계 10·11 초기안 대체), 진입 팝업 바텀시트(MySubmissionsPopupGate) 제거,
> 홈 헤더 자산추이 버튼 하단 이동. 상세는 `docs/decisions.md` 2026-07-22 결정 ④.
