...
> 완료: 2026-06-18 (코드/마이그레이션/테스트/문서). prod 롤아웃은 미실행 — 아래 운영 작업 참조.
...
- [x] code-review(high) 후속 4건 반영 (pnl_sync user_id scope 외)

**남은 운영 작업 (미실행, 사용자 실행):**
- [ ] (운영) prod 롤아웃 — 코드 배포 → `psql -U postgres`로 `0002_drop_rls` 적용+alembic_version 갱신 → 코드 배포 → Coolify `ADMIN_DATABASE_URL` 제거
