...
- [x] Step 1 — `db_ops/trades_repo.py`에서 `list_trades_in_range` 삭제
- [x] Step 1 — `domain/holdings.py`에서 `compute_lot_quantity`, `find_latest_buy_strategy`, `compute_flexible_holding_days` 삭제. 사용하지 않게 된 import (`math`, `MS_PER_DAY`, `to_kst`, `trade_to_group_key`) 정리. **`is_same_group`은 살아 있는 `compute_holding_summary`에서 계속 사용하므로 유지**.
- [x] Step 1 — `tests/test_holdings.py`에서 `TestComputeLotQuantity`, `TestComputeFlexibleHoldingDays`, `TestFindLatestBuyStrategy` 클래스 + 해당 import 제거
- [x] Step 1 — `cd api && poetry run pytest -q` 통과 + commit
- [x] Step 2 — `domain/trade_utils.py`에 `to_kst_ms(dt: datetime) -> int` 추가
- [x] Step 2 — `domain/realized_pnl.py:146`, `domain/trade_walker.py:115`에서 `to_kst_ms` 사용
- [x] Step 2 — `cd api && poetry run pytest -q` 통과 + commit
- [x] Step 3 — `utils/numbers.py`에 `half_up_int(x: float) -> int` 추가
- [x] Step 3 — `domain/realized_pnl.py:131` 인라인 표현 교체
- [x] Step 3 — `domain/analysis/rules.py`의 private `_round` 제거하고 호출부 모두 `half_up_int`로 교체
- [x] Step 3 — `cd api && poetry run pytest -q` 통과 + commit
- [x] Step 4 — `domain/trade_utils.py`에 `position_key(ticker: str | None, country: str) -> str` 추가
- [x] Step 4 — `domain/portfolio.py:150,243`, `domain/analysis/concentration.py:60`에서 `position_key` 사용
