...
- [x] `api/src/invest_note_api/external/naver_search.py` 신규 — `_search_kr` 추출 + `find_first_kr_match(q)` helper
- [x] `api/src/invest_note_api/routers/stocks.py` — 새 helper로 위임 (테스트 mock 경로도 함께 갱신 고려)
- [x] `api/src/invest_note_api/broker_import/ticker_resolver.py` — stocks_repo 의존 제거 + Naver 단일 경로 + 정확도 가드 + asyncio.gather 병렬
- [x] `api/src/invest_note_api/routers/trades.py:420` — 미해결 에러 메시지 갱신
- [x] `api/src/invest_note_api/db_ops/stocks_repo.py` 삭제
- [x] `api/scripts/seed_stocks.py` 삭제
- [x] `supabase/migrations/016_drop_stocks.sql` 신규 — `drop table if exists public.stocks`
- [x] `app/src/components/records/ImportTradesPanel/PreviewStep.tsx:79` — 미해결 안내 문구 갱신
- [x] `api/tests/test_ticker_resolver.py` 신규 — Naver mocking으로 매칭/가드/실패 케이스 단위 테스트
- [x] `api/tests/test_stocks.py` 기존 테스트 회귀 확인 (mock 경로가 `_search_kr`인지 새 helper인지 — 변경 시 함께 수정)
- [x] `docs/decisions.md` — 결정 기록 추가
- [x] `docs/backlog.md` — "마스터 재도입" 장기 후보 기록
- [x] `cd api && poetry run pytest -q` — 백엔드 테스트 통과
- [x] `pnpm tsc` — 타입 체크 통과
