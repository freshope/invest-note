...
**Phase 1 — BE 데이터층**
- [x] `026_daily_close_prices.sql` (stocks RLS/grant 미러) → verify: 마이그레이션 적용·테이블 생성
- [x] `daily_price_seed.py` fetcher → verify: 실측 응답 shape mock 단위테스트
- [x] `daily_prices_repo.py` (watermark/get/upsert/prune) → verify: 단위테스트/쿼리 리뷰

**Phase 2 — BE 엔드포인트**
- [x] `domain/asset_history.py` → verify: 합성 거래+종가 단위테스트
- [x] `schemas/asset_response.py` + `routers/assets.py` + `main.py` 등록 → verify: `GET /assets/history` pytest
- [x] (옵션) admin `POST /admin/seed/daily-prices` → verify: 202

**Phase 3 — FE**
- [x] api-client `assetsApi.history` + query-keys + 타입 + `useAssetHistory` → verify: tsc
- [x] `AssetHistoryChart.tsx` 커스텀 팬 차트 → verify: 렌더·스와이프 팬
- [x] `AssetHistoryList.tsx` + `AssetHistoryPage.tsx` + `app/(app)/assets/page.tsx` → verify: 양쪽 모드 로드
- [x] 홈 헤더 진입 아이콘 → `/assets` → verify: 네비게이션
- [x] StockDetail 종목명 옆 아이콘 → `/assets?ticker=` → verify: 네비게이션

**Phase 4 — 검증**
- [x] `pnpm -C fe exec tsc --noEmit` + `cd be && poetry run pytest -q` 통과
- [x] gstack 으로 계좌뷰/종목뷰 진입·차트 팬·목록 실측
