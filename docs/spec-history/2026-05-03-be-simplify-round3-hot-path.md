...
### 사전 — 문서 동기화
- [x] `docs/decisions.md` 에 Item 5 (analysis period SQL push) 미진행 결정 기록
- [x] `docs/backlog.md` "효율 / 핫패스" 섹션에서 Item 5 줄 제거 (decisions.md 참조)
- [x] `docs/issue-current.md` 도 본 plan 내용으로 동기화

### Item 1 — httpx 공유 client
- [x] `main.py` — lifespan 에 `app.state.http_client = httpx.AsyncClient(timeout=HTTP_TIMEOUT_SECONDS)` 추가, 종료 시 `aclose()`
- [x] `external/http_client.py` 신규 — `create_http_client()` + `get_http_client(request)` Depends helper
- [x] `external/quotes.py` — `fetch_quotes_by_keys(state, keys, *, client)` 시그니처로 client 받음 (필수), 내부 `async with httpx.AsyncClient()` 제거
- [x] `external/naver_search.py` — 이미 `client` 키워드 옵션 받음. `client=None` fallback 은 단위 테스트 호환성을 위해 유지 (production 라우터는 항상 client 전달)
- [x] `broker_import/ticker_resolver.py` — `resolve_tickers(asset_names, ticker_hints, *, client)` 시그니처로 client 받음. 호환을 위해 `client=None` fallback 유지 (라우터 production 경로는 항상 client 전달)
- [x] 라우터들 (`routers/trades.py`, `routers/stocks.py`, `routers/portfolio.py`, `routers/analysis.py`) — `Depends(get_http_client)` 로 주입해 호출 함수에 전달

### Item 2 — broker parser threadpool
- [x] `routers/trades.py:340` — `parse_result = await run_in_threadpool(parser.parse, file_bytes, filename)` 래핑

### Item 4 — list_trades date 범위 옵션 (import_preview 적용)
- [x] `db_ops/trades_repo.py` — `list_trades` 에 `*, date_from: datetime | None = None, date_to: datetime | None = None` 키워드 옵션 추가, ORDER BY DESC 유지
- [x] `routers/trades.py:352` `import_preview` — 파싱 결과의 KST 일자 min/max 계산 → `kst_date_to_utc` 로 UTC 변환 → `list_trades(date_from=min_dt, date_to=max_dt)` 로 호출. 빈 파싱 결과는 호출 자체 회피
- [x] 빈 파싱 결과/모두 ticker 미해결 시 `existing_sigs` 가 빈 셋이어도 동작 동일함 확인

### Item 6 — delete_account round-trip 통합
- [x] `routers/accounts.delete_account` — `DELETE FROM accounts WHERE id = $1 AND user_id = $2 AND NOT EXISTS (SELECT 1 FROM trades WHERE account_id = $1) RETURNING id` 통합. row 없을 때 분기: account 미존재 → 404, 거래 잔존 → 409

### 검증
- [x] 타입 체크: 프로젝트 표준 명령 (확인 후 `mypy` / `ruff` / 기타)
- [x] 테스트 통과: `cd api && poetry run pytest -q`
- [x] 핵심 회귀 케이스 점검: `tests/test_trades_router.py` (import_preview), `tests/test_accounts.py` (delete 분기), `tests/test_quotes*.py` / `tests/test_stocks*.py` (공유 client)

### 마무리 (spec-finish 시 처리)
- [x] `docs/backlog.md` "효율 / 핫패스" 섹션에서 처리된 4 개 항목 제거 + Round 4 (페이지네이션, FE 협조) 메모 유지
- [x] `docs/issue-current.md` → `docs/issue-history/2026-05-03-be-simplify-round3-hot-path.md` 이동
