...
- [x] `api/pyproject.toml`에 `asyncpg` 추가 + `poetry lock --no-update`
- [x] `api/.env.example`에 `DATABASE_URL=` 템플릿 추가
- [x] `api/src/invest_note_api/config.py`에 `database_url` 필드 추가
- [x] `api/src/invest_note_api/errors.py` 작성 (APIError + 전역 핸들러)
- [x] `api/src/invest_note_api/auth/dependency.py` HTTPException → APIError 교체
- [x] `api/src/invest_note_api/db.py` 작성 (pool, get_pool, acquire_for_user)
- [x] `api/src/invest_note_api/schemas/account.py` 작성 (AccountCreate, AccountUpdate)
- [x] `api/src/invest_note_api/main.py` — lifespan + handler + router 등록
- [x] `api/src/invest_note_api/routers/accounts.py` — 5개 엔드포인트 구현
- [x] `api/tests/fake_pool.py` 작성
- [x] `api/tests/conftest.py` — pool override fixture 추가
- [x] `api/tests/test_accounts.py` — 5개 라우트 핵심 케이스
- [x] `poetry run pytest` 전체 통과
- [x] `api/README.md` — DB 설정 + curl 예시 갱신
