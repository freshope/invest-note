...
- **`OTA_REQUIRED_NATIVE` 1.1.23 → 1.5.0 인상 완료 (2026-08-10)** — v1.5.0_34 양 스토어 라이브 확인 후 이번 번들 발행 전에 갱신(직전 릴리즈 노트의 미이행 지시사항 해소). 이후 **1.4.3_33 이하 기기는 모든 OTA 에서 제외** — 실측 확인: 네이티브 1.4.3 요청 시 `up_to_date` 반환. 다음 네이티브 제출 시 다시 그 마케팅 버전으로 인상 필요
- **v1.5.1 번들 발행 완료 (2026-08-10)** — checksum `5ee0183c…ce7d0`, manifest flip 및 앱 경로(`POST /live-update/manifest`) 응답 검증 완료
