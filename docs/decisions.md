...
## 2026-07-28 | 푸시 전송 — 직접 FCM+APNs 2채널 → Firebase Admin 단일 채널 (2026-07-22 결정 ③ 대체)

- **맥락:** 2026-07-22 에 배선한 `push_sender.py` 는 Android(FCM HTTP v1: 서비스계정 JWT 수동 서명 → OAuth2 교환 → REST)와 iOS(`api.push.apple.com` 에 `.p8` ES256 JWT 직접 서명 → HTTP/2)를 각각 직접 구현했다. 실전송은 한 번도 검증되지 않은 상태(PostHog push 0건)에서 활성화를 준비하다, 시크릿 6개 + iOS `aps-environment`↔APNs 호스트(sandbox/prod) 수동 매칭이 조용히 실패하기 쉬운 구조임이 드러났다.
- **결정 ① 어댑터 2개 직접 구현 → `firebase-admin` SDK 단일 채널.** `services/push_sender.py` → `services/push/` 패키지(`base` Protocol + `firebase` + `dummy`). iOS APNs 는 Firebase 가 위임 발송한다.
  - **이유:** env 시크릿 6개 → 1개(`GOOGLE_APPLICATION_CREDENTIALS_JSON`), APNs 호스트·entitlement 매칭 고민 제거(Firebase 자동 라우팅 — `aps-environment` 는 `development` 유지). 같은 워크스페이스 today-alive `api/app/services/push/` 에서 이미 검증된 구조.
  - **트레이드오프:** FE 플러그인도 `@capacitor/push-notifications` → `@capacitor-firebase/messaging` 로 바꿔야 해서(Firebase Admin 은 raw APNs 토큰을 못 받고 FCM 토큰만 받는다) **네이티브 릴리즈 + 양 스토어 재심사**가 강제된다. 2026-07-03 선통과 배치가 확보해둔 무심사 OTA 경로를 이번 건에 한해 포기한 셈.
- **결정 ② 게이트는 `push_enabled`(자격증명 존재) 하나 — `PUSH_PROVIDER` env 미도입.** 어댑터 교체는 `send_to_user(adapter=...)` 인자 주입(테스트용)으로만 노출한다.
  - **이유:** today-alive 는 `PUSH_PROVIDER` + 팩토리를 두지만, 자격증명 유무 게이트와 provider 게이트가 서로 어긋날 수 있다. 항상 한 종류만 반환하는 팩토리는 단일 사용처 추상화라 지연 import 로 대체.
  - **트레이드오프:** 나중에 provider 를 실제로 늘리면 팩토리를 다시 만들어야 한다.
- **결정 ③ base64 시크릿 우회 제거.** 2026-07-22 에 넣은 `_decode_b64_secret` validator 를 없애고 env 값을 그대로 쓴다.
  - **이유:** 멀티라인이라 `.env` 파서를 깨뜨리던 실체는 APNs `.p8` PEM 이었다. 서비스계정 JSON 은 `private_key` 의 개행이 JSON `\n` 이스케이프라 **원래 한 줄로 표현된다** → `.p8` 이 사라지면서 우회 이유도 함께 사라짐.
  - **트레이드오프:** 운영 env 를 한 줄 JSON 으로 다시 넣어야 한다(Coolify SSOT).
- **결정 ④ 폐기 토큰 정리 배선 신규 — `token_invalid` → `device_tokens` 삭제.** `UnregisteredError`/`SenderIdMismatchError`/`InvalidArgumentError` 3종을 모두 폐기로 매핑하고, 네트워크·quota 실패는 토큰을 보존한다.
  - **이유:** 기존 코드엔 정리 경로가 없어 경고 로그만 쌓였다. 특히 cutover 이전에 저장된 **iOS raw-APNs 토큰(64 hex)을 FCM 에 넘기면 `UnregisteredError` 가 아니라 `InvalidArgumentError`** 가 온다 — 이 매핑이 없으면 구 토큰이 영구히 남아 매 통지마다 실패한다. 별도 backfill 마이그레이션 없이 첫 전송에서 자연 정리된다.
  - **트레이드오프:** 재시도 가능한 실패와 폐기를 잘못 구분하면 살아있는 토큰을 지운다 → 두 경우를 테스트로 고정.
- **함정 기록:** ① `firebase-admin` 이 `httpx[http2]==0.28.1` 을 하드핀해 레포 `httpx ^0.27` → `^0.28` 동반 상향 불가피(사용처가 전부 표준 형태라 무회귀, 전체 테스트로 확인). ② `firebase` npm 은 optional peer 인데도 플러그인 web 구현이 `firebase/messaging` 을 정적 import 해서 next build 에 필수. ③ `messaging.send()` 와 어댑터 생성(서비스계정 키 파싱)이 blocking 이라 async 라우트에서 `asyncio.to_thread` 래핑 필요(today-alive 는 전스택 sync 라 해당 없음 — faithful copy 금지). ④ iOS `GoogleService-Info.plist` 는 파일 배치만으론 부족하고 Xcode 타깃 Copy Bundle Resources 등록 필요.
- 참조: `docs/spec-current.md`, [[project_push_firebase_migration]], [[feedback_release_scope_native_plugin_gap]], [[project_multiline_secret_base64_coolify]].
...
- **결정 ③ Phase 2 push sender 는 FCM/APNs 시크릿(env) 없으면 조용히 no-op(게이트).** *(→ 전송 방식은 2026-07-28 Firebase Admin 단일 채널로 대체. no-op 게이트 사상 자체는 유지.)* `services/push_sender.py` 는 자격증명이 없으면 로그만 남기고 skip — PostHog no-op 계약 사상. producer 훅은 best-effort(sender 실패가 통지 insert/응답을 깨지 않음).
...
- **결정 ① 제거/유지 경계 — "검증 fallback"만 제거, "계정삭제 IdP 관리"는 유지.** config 의 `SUPABASE_*` 중 **OAuth 중개용은 없다**(중개는 Google/Kakao/Apple 직결). 따라서 경계는 검증 vs IdP-관리로 갈린다. **제거(검증):** `decode_oidc_jwt` 의 `supabase_entry` default fallback 분기 + 레거시 단일-인자 경로(`jwks_uri`/`audience`/`issuer`/`supabase_entry` 파라미터 → 시그니처가 `decode_oidc_jwt(token, *, registry)` 로 축소), `config.oidc_issuer`·`oidc_audience` 필드, `config.jwks_uri`·`supabase_issuer_entry` property. **유지(검증 아님):** `supabase_url`·`supabase_secret_key`·`auth/identity_provider.py:delete_user` — 회원 탈퇴 시 레거시 Supabase 신원 제거(IdP 관리, me.py)다. 그 물리 제거는 runbook 2c "비가역 클라우드 정리"(Supabase 프로젝트/`SUPABASE_*` env/`supabase/` 디렉토리)에 묶여 본 작업 범위 밖. 탈퇴 502 는 FE 가 정상 종료 처리([[project_account_deletion_audit]]).
- **결정 ② orphan REMOVE — JWKS HTTP fetch 경로 물리 제거.** fallback 제거 후 런타임 registry 는 BE-only 이고 모든 entry 가 `_registry_with_be_key` 로 in-process `verify_key` 를 주입받는다 → JWKS fetch 경로가 도달 불가. `_get_jwks_client`·`from jwt import PyJWKClient`·`_verify_with_entry` 의 `verify_key is None` else 분기·`dependency.py` 의 `PyJWKClientError` except 절을 **제거**(`entry["verify_key"]` 직접 사용). 보존 시 반쯤 죽은 코드 + 가상 비-BE issuer 용 speculative 테스트 부채(CLAUDE.md "no speculative flexibility"). `be_jwks_uri` property 는 registry BE entry 의 `jwks_uri` 키를 harmless nominal 메타로 보존하는 용도로만 유지(향후 클라우드 정리 시 `be_oauth_redirect_base` 기준 재유도 필요 — forward 코멘트).
- **결정 ③ ⚠️ 검증 불변식 의도적 역전 — registry 비면 전원 401.** 이전 안전속성 "registry 비면 Supabase entry 단독 = Phase1 동일 = dormant-safe(구 앱 무회귀)"를 **폐기**한다. 이제 `be_token_signing_key` 미설정 → `oidc_issuer_registry` 빈 → iss 미매칭/누락 토큰은 전부 `jwt.InvalidTokenError → 401`. 즉 **BE signing key 가 인증의 hard precondition**(로컬 dev `.env.local` 에 키 없으면 dev 인증 전멸). 가드 테스트 `test_issuer_registry::test_case5_dormant_registry_rejects_all` 로 박제.
- **결정 ④ 방침 전환 "웹 폐기" → "개발 편의용 웹 BE flow 복구"(어드민 client 패턴 미러링).** 초안은 웹 분기 자체를 삭제(웹 비배포 타깃, [[project_deploy_targets]])하려 했으나, 사용자 방침 변경으로 **웹도 BE OAuth flow** 로 동작시키되 supabase-js 는 물리 제거. 어드민 패널이 이미 구현한 web BE-flow([[project_admin_panel]], 2026-06-26 어드민 BE-auth 결정)를 `app/` 에 미러링: BE `/auth/login` 에 `client="web"` 분기 + `_handle_callback` web 분기 신규, config `be_app_web_redirect_url`(default `""`). `client=web` 인데 env 빈 값이면 **dormant-503**(어드민 `client=admin` 가드와 대칭, surgical 추가 — map 리팩토링 안 함). 어드민과 유일 차이는 client 값(`admin`→`web`)·redirect env 뿐, param/엔드포인트 동일. **운영은 dormant**(env 비움) → 운영 보안 표면 증가 0, IdP redirect_uri 는 BE callback 고정(2차 hop)이라 IdP 콘솔 등록 불요. FE 6 auth 함수는 supabase 분기 제거 후 **platform 분기**(웹=full-page redirect+https callback, 네이티브=Browser.open+딥링크)로 단일화 — `AuthStrategy` 인터페이스는 신설 안 함(과설계, backlog ⑤ 단순 단일화로 충족).
- **결정 ⑤ ⚠️ C5 예외 — 웹 토큰 localStorage 영속(네이티브 secure storage 불변).** `app` 의 `token-store.ts` 는 "평문 localStorage 금지(C5, 금융 앱)" 원칙 하에 네이티브 secure storage(Keychain/Keystore) 전용이었다. 웹 BE flow 가 동작하려면 verifier 가 full-page redirect 왕복을, access/refresh 가 리로드를 생존해야 해 **웹 경로는 localStorage** 가 불가피 → `token-store.ts` 를 platform 분기(네이티브=secure storage 현행 유지·lazy import 는 네이티브 분기 내, 웹=localStorage·어드민 미러링). C5 예외 근거 3개를 코멘트로 박음: ⓐ 웹은 **개발 전용**(운영 dormant), ⓑ 웹은 **배포 타깃 아님**, ⓒ 어드민이 동일 선례(내부 콘솔). 옛 supabase-js 도 localStorage 였으므로 parity(신규 위험 아님).
- **결정 ⑥ 배포 성격 분리 — BE-#1(fallback 제거)만 게이트, 나머지는 무관.** **BE-#1 은 별개 커밋**: 배포 즉시 Supabase 토큰 보유 구앱(2026-06-29 측정 활성 26.2%)이 락아웃되므로 "⚠️ legacy 구앱 비중 ~0 수렴(force-update 전파) 전 배포 금지". 판정은 PostHog 재측정(runbook 2c 로그). **BE-#3(web client)·FE-#2 는 legacy 게이트 무관**(web client=운영 dormant, FE=출하 build31 무영향·미래 빌드만). BE-#1 과 BE-#3 는 둘 다 `config.py` 를 건드리나 라인 무겹침(제거 vs 추가)이라 순차 커밋으로 분리(hunk 수술 불요).
- **트레이드오프/잔여:** ⓐ orphan REMOVE 로 `_verify_with_entry` 가 `entry["verify_key"]` 하드 subscript 사용 → 미래에 verify_key 없는 비-BE issuer 가 registry 에 추가되면 401 아닌 `KeyError → 500`. 해당 issuer 가 없어 latent, 추적은 거부한 speculative infra(미래 비-BE JWKS issuer 도입 시 검증 경로 의도적 재도입). ⓑ 크로스탭 storage 동기화(어드민 보유)는 단일 dev 탭 가정으로 제외(YAGNI). ⓒ `delete_user` 가 Supabase 에 의존하는 한 BE-flow 신규 가입자(Supabase 신원 없음) 탈퇴 시 admin deleteUser 가 404→502 가능 — 기존 설계대로 FE 가 502 를 정상 종료 처리(데이터는 이미 삭제). 클라우드 정리 단계에서 `delete_user` 자체를 제거/대체. ⓓ 검증: 전체 pytest **949 passed/10 skipped**, FE `tsc 0 + 259 tests green`, QA-#1/#2 🟢(차단 0). 참조: [[project_auth_decoupling]]·[[project_auth_cutover_exec_method]]·[[project_admin_panel]]·[[project_supabase_oauth_only]].
