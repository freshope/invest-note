...
def test_create_account_empty_name(accounts_client):
    conn = FakeConnection()
    with _patch(conn):
        r = accounts_client.post("/v1/accounts", json={"name": "  ", "cash_balance": 0})
...
def test_update_account_empty_body_returns_204(accounts_client):
    conn = FakeConnection()
    with _patch(conn):
        r = accounts_client.patch(f"/v1/accounts/{ACC_ID}", json={})
...
def test_delete_account_success(accounts_client):
    ...
    with _patch(conn):
        r = accounts_client.delete(f"/v1/accounts/{ACC_ID}")
    assert r.status_code == 204
...
def test_trade_count_success(accounts_client):
    ...
    with _patch(conn):
        r = accounts_client.get(f"/v1/accounts/{ACC_ID}/trade-count")
    assert r.status_code == 200
