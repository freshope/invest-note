...
class TestStocksQuote:
    ...
    def test_quote_us_ok(self, trades_client):
        async def mock_quotes(keys):
            return {"AAPL:US": {"price": 185.0, "currency": "USD", "as_of": "2024-01-15"}}

        with patch("invest_note_api.routers.stocks.fetch_quotes_by_keys", mock_quotes):
            resp = trades_client.get("/api/stocks/quote", params={"symbols": "AAPL:US"})

        assert resp.status_code == 200
        assert resp.json()["AAPL:US"]["price"] == 185.0
...
class TestStocksSearch:
    ...
    def test_search_us_by_english(self, trades_client):
        async def mock_search_us(q):
            return [{"code": "AAPL", "name": "Apple Inc.", "market": "US", "exchange": "NASDAQ"}]

        with patch("invest_note_api.routers.stocks._search_us", mock_search_us):
            resp = trades_client.get("/api/stocks/search", params={"q": "apple"})

        assert resp.status_code == 200
        assert resp.json()[0]["code"] == "AAPL"
        assert resp.json()[0]["market"] == "US"
