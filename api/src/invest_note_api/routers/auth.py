...
from invest_note_api.errors import ERR_REQUEST_FALLBACK, ERR_UNAUTHORIZED, APIError
...
def _pkce_verify(challenge: str, verifier: str) -> bool:
    """앱↔BE PKCE(Layer2, B12) — S256(verifier) == 저장된 challenge 인지 대조."""
...
async def callback_get(
    ...
    return await _handle_callback(code, state, settings, pool)
...
async def callback_post(
    ...
    return await _handle_callback(
        str(code), str(state), settings, pool,
...
async def _handle_callback(
    ...
    *,
    ...
    sep = "&" if "?" in settings.be_deeplink_scheme else "?"
    return RedirectResponse(
        f"{settings.be_deeplink_scheme}{sep}{urlencode({'code': one_time})}",
        status_code=302,
    )
...
async def token(
    ...
    # challenge 누락/불일치 = 거부(enforce-always, 잔여위험 수용X).
    challenge = payload.get("app_code_challenge")
    if not challenge or not _pkce_verify(challenge, body.code_verifier):
        raise APIError(ERR_UNAUTHORIZED, 401)
