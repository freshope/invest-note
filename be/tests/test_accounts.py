...
def test_list_accounts_empty(accounts_client):
    conn = FakeConnection([], [])
    with _patch(conn):
        r = accounts_client.get("/accounts")
...
def test_create_account_empty_name(accounts_client):
    conn = FakeConnection()
    with _patch(conn):
        r = accounts_client.post("/accounts", json={"name": "  ", "cash_balance": 0})
...
def test_update_account_empty_body_returns_204(accounts_client):
    conn = FakeConnection()
    with _patch(conn):
        r = accounts_client.patch(f"/accounts/{ACC_ID}", json={})
...
def test_delete_account_success(accounts_client):
    ...
    with _patch(conn):
        r = accounts_client.delete(f"/accounts/{ACC_ID}")
    assert r.status_code == 204
