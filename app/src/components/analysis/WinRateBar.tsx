...
export function WinRateBar({ rate, hasData, emptyLabel = "결과 없음" }: WinRateBarProps) {
  ...
  const color = rate >= WIN_THRESHOLD ? "bg-[var(--rise)]" : rate < LOSS_THRESHOLD ? "bg-[var(--fall)]" : "bg-amber-400";
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={cn("text-[12px] tabular-nums font-semibold w-8 text-right",
        rate >= WIN_THRESHOLD ? "text-[var(--rise)]" : rate < LOSS_THRESHOLD ? "text-[var(--fall)]" : "text-amber-500",
      )}>
        {Math.round(rate)}%
      </span>
    </div>
